...
) const
{
    if (cellAnchorPoints[celli].size())
    {
        ...
        // Maybe we have already a refined face so check all the face
        // vertices.
...
)
{
    ...
    const scalar s = (fcToOwn&n) / (dir&n);
    ...
    if (changed && haveTwoAnchors)
    {
        ...
        forAll(cFaces, i)
        {
            ...
            if (faceMidPoint[facej] != faceMidPointi
                ...
                && faceMidPoint[facej] != 1234567890) // LC could be replaced by facej == type "empty" or "wedge"
            {
...
) const
{
    ...
    // LC: from edge mid to anchor points, in 2D 8 in enought
    ...
    forAll(cFaces, i)
    {
        ...
        if (nAnchors == 1) //to be checked
        {
            ...
            // LC: in 2D case this should never happen, as the shared faces with uncutted mesh
            ...
            // LC: these two lines just repeat the same steps two times to reach the second cLevel+1
        ...
        else if (nAnchors == 2)
        {
            ...
            // been split using cLevel+1 and cLevel+2 points.
            faceMidPointi = 1234567890;        //LC: magic number is not a nice solution
        }
        else if (nAnchors == 4 || nAnchors == 3)
        {
            // There is no face middle yet but the face will be marked for
            // splitting.
            // LC: For non-empty/wedge faces, faceMidPoint[faceI] will be 1234567890,
          ...
          forAll(f, fp0)
          {
              ...
              if (pointLevel_[point0] <= cLevel)
              {
                  ...
                  else
                  {
                      ...
                      // LC: in 2D this should never be used as this is verified with
                  ...
                  else
                  {
                      ...
                      // LC: in 2D this should never be used as this case is verified with
                      // shared faces between refined and not-refined cells which have 
                      // to be refined in this turn.
                      label edgeMid = findLevel
                      (
...
)
{
    ...
    forAll(cellLabels, i)
    {
        ...
        cellMidPoint[celli] = 1234567890;    //LC: mark need for splitting
    }
    ...
    forAll(cellMidPoint, celli)
    {
        if (cellMidPoint[celli] >= 0)
        {
            ...
            forAll(cEdges, i)
            {
                ...
                {
                    //LC: fake value as in the original code
    ...
    // - LC: isDivisibleFace true : face needs to be split in 4 else in 2.
...
Foam::labelList Foam::hexRef2D::getSplitEdges() const
{
    ...
    // LC: Unmark all with not 4 cells (so internal edges)
    ...
    // LC: this is probably a redundant check (parallel maybe?)
    for
    (
...
) const
{
    ...
    bitSet unrefineEdge(mesh_.nEdges(), edgesToUnrefine);
...
bool Foam::hexRef2D::write(const bool valid) const
{
    bool writeOk =
...
void Foam::hexRef2D::removeFiles(const polyMesh& mesh)
{
    IOobject io
    (
        "dummy",
